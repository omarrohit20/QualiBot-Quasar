...
Every automated run — API and UI, on every platform — executes on Chrome only, one test at a time. `playwright.config.ts` is configured with `fullyParallel: false` and `workers: 1`, and only the `chromium` project is defined (the earlier `firefox`/`webkit` API projects were removed). This was a deliberate fix: running chromium/firefox/webkit concurrently against the shared OrangeHRM demo site caused each browser's login to invalidate the others' session, producing intermittent, environment-caused test failures that looked like application bugs. Never pass `--workers` above 1 or `--project=firefox`/`--project=webkit` when invoking the CLI.
...
- **All test execution is sequential and Chrome-only.** `playwright.config.ts` runs with `workers: 1` and `fullyParallel: false`, and only a Chrome-based project is defined — never invoke the CLI with extra workers or a non-Chrome `--project`. This is what fixed the concurrent-session failures seen when chromium/firefox/webkit logins raced each other on the shared demo environment.
