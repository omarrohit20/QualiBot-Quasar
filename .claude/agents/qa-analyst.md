...
### Always sequential, Chrome-only — no parallel workers, no cross-browser fan-out

Every automated run — API and UI, every platform (Claude, Codex, Cursor, GitHub) — executes on Chrome only, one test at a time.

- `playwright.config.ts` must have `fullyParallel: false` and `workers: 1`. If you find it configured otherwise (parallel workers, or `firefox`/`webkit`/other-browser projects), that is a defect in the config — fix it, don't work around it by passing ad hoc flags.
...
- When running tests via the Playwright CLI (`npx playwright test ...`), never pass `--workers` above 1 and never target more than the Chrome project.
...
- **Run API and UI tests as two separate CLI invocations, never combined into a single command.** API first: `npx playwright test spec/api/...` (or `--grep "<title>"` scoped to API specs). Then UI: `npx playwright test spec/ui/...` (or `--grep "<title>"` scoped to UI specs). Keeping them separate makes it obvious which layer a given failure came from and keeps each run's `playwright-report`/`test-results` output distinct before you merge the findings back into one report in step 7 — don't invoke both in one `npx playwright test` call. Both respect `ENV`, run headless — see "Always headless" under Tooling. Never add `--headed`.
