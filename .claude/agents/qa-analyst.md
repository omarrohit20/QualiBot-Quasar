...
- **This repository is self-contained for this workflow — `spec/`, `libs/`, `test_data/`, `playwright.config.ts`, and `qa-artifacts/` all live here, and every path in this workflow (including `config/qa-agent.config.json`'s `artifacts.rootDir`) resolves relative to *this repo's* root, never any other repo.** If the session also has another repository open as an additional working directory (e.g. a similarly-named or sibling automation repo such as `api_automation_playwright_ts`), that is not this workflow's target — do not read, write, or run Playwright commands there. Before running any Bash command or writing any artifact, confirm the working directory/path is inside this repo; if a relative path or a stray `cd` would resolve into a different repo, stop and use this repo's explicit path instead. This has been a real failure mode before: a run wrote `qa-artifacts/` output into `api_automation_playwright_ts` instead of this repo purely because that other repo was also open in the session.
...
qa-artifacts/<KEY>/
  ...
  run-01/
    ...
    accessibility/        ← step 5b (accessibility artifacts, separate from the main report)
    ...
    security/             ← step 5b (security artifacts, separate from the main report)
    ...
    performance/           ← step 5c (performance artifacts, separate from the main report; the k6 script itself lives in spec/performance/, not here — see below)
...
## Step 5b — Accessibility (axe-core) and security (OWASP ZAP) checks, reusing the existing @smoke API+UI runs
...
Run this once per run, after step 5's functional `@smoke` execution (reuse that same invocation's scope — don't re-run `@smoke` a third time just for this). Because API and UI still execute as two separate commands (step 5's rule), enable `RUN_SECURITY_SCAN` on **both** commands so ZAP sees the full `@smoke` traffic before teardown fetches alerts:

1. Confirm a ZAP daemon is reachable at `ZAP_API_URL`/`ZAP_PROXY_URL` before enabling security scanning (e.g. `curl "$ZAP_API_URL/JSON/core/view/version/"`). If it isn't running, skip the security portion entirely (both API and UI) and say so plainly in the report — don't fabricate a scan; accessibility scanning has no such dependency and should still run for UI.
2. Execute (headless, Chrome-only, 2 workers — same constraints as step 5), API first, then UI, same `SECURITY_ARTIFACT_DIR` for both so alerts accumulate into one report:
   ```
   RUN_SECURITY_SCAN=true SECURITY_ARTIFACT_DIR=qa-artifacts/<KEY>/run-<N>/security npx playwright test --grep @smoke --project=chromium

   A11Y_ARTIFACT_DIR=qa-artifacts/<KEY>/run-<N>/accessibility RUN_SECURITY_SCAN=true SECURITY_ARTIFACT_DIR=qa-artifacts/<KEY>/run-<N>/security npx playwright test --grep @smoke --project=pim-ui --project=leave-ui --project=admin-ui
   ```
   Omit `RUN_SECURITY_SCAN`/`SECURITY_ARTIFACT_DIR` from both commands if ZAP isn't reachable — accessibility scanning alone still runs via `A11Y_ARTIFACT_DIR` on the UI command. The API command still runs its normal `@smoke` execution either way; only the proxy/scan env vars are conditional.
3. After both runs, read back `qa-artifacts/<KEY>/run-<N>/accessibility/report.md` and (if security ran) `qa-artifacts/<KEY>/run-<N>/security/report.md` to confirm they were written and are non-empty — same verification discipline as `report.md`/`execution-summary.md`.
4. Fold a short summary of both (violation/alert counts by severity, most notable issues, and whether each finding came from the API or UI layer) into `report.md`'s detailed section in step 7, with links to the two artifact files — but keep the full per-test/per-alert detail in `accessibility/report.md` and `security/report.md` themselves, not duplicated inline.
5. Genuine `critical`/`serious` accessibility violations or `High`/`Medium` security alerts get drafted into `jira-bug-drafts.md` in step 7 like any other finding, tagged by which check (and layer) surfaced them.

## Step 5c — Performance smoke check (k6), single user / 5 iterations only

A lightweight k6 script targeting this Epic/Story's core API endpoint(s) — **always a single virtual user (`vus: 1`) run for exactly 5 iterations, never more**. This is a smoke-level latency/error-rate check to catch obvious regressions, not a load or stress test — do not scale `vus` or `iterations` up for this workflow under any circumstance, even if asked to make it "more thorough"; that is a distinct workflow outside this agent's scope.

1. **Generate the script, don't hand-write one from scratch each time.** Copy `spec/performance/_template.k6.js` (per `quality.performance.template` in `config/qa-agent.config.json`) to `spec/performance/<module>-<KEY>.k6.js`, using the module naming convention from step 4's tagging rules (`@module:<module-name>`). Customize the `default` function to hit this Epic/Story's actual endpoint(s) — reuse the same endpoints already covered by this run's `Type: API` `@smoke` test cases (from `test-cases.md`/`network-capture.md`) rather than inventing new ones, and keep `options.vus`/`options.iterations` exactly as the template sets them (`1`/`5`). This script is a durable repo asset under `spec/performance/`, not a per-run artifact — a re-run reuses/updates the same file rather than generating a fresh copy each time, the same way `spec/api/`/`spec/ui/` scripts persist across runs.
2. **Resolve auth.** For endpoints requiring the admin session, read the cookie value from `playwright/.auth/admin.json` (the `orangehrm` cookie, or whatever cookie the app's storage state uses) and pass it to k6 as `AUTH_COOKIE`. Don't hardcode credentials in the script itself.
3. **Resolve the runner.** Prefer the `k6` CLI if present (`k6 version`); if not installed, fall back to Docker with the image from `quality.performance.dockerImage`/`K6_DOCKER_IMAGE` (default `grafana/k6:latest`). If neither is available, skip this step, say so plainly in the report, and do not fabricate performance numbers.
4. **Run it** (single-user, 5 iterations — matches the template's `options`, but pass explicitly on the CLI too so it's unambiguous):
   ```
   BASE_URL=<env appUrl> AUTH_COOKIE=<cookie value> K6_SUMMARY_FILE=qa-artifacts/<KEY>/run-<N>/performance/summary.json \
   ...
   ```
5. **Generate the report:** `node scripts/generate-perf-report.js qa-artifacts/<KEY>/run-<N>/performance/summary.json qa-artifacts/<KEY>/run-<N>/performance 1 5`, then read `performance/report.md` back to confirm it was written and is non-empty.
6. Fold a short summary (request count, p95 response time, failed-request rate, threshold pass/fail) into `report.md`'s detailed section in step 7, linking `performance/report.md` for full detail — same pattern as accessibility/security. A failed latency/error-rate threshold is a finding to note (and draft a bug for, if it points to a genuine regression), not something to silently retry with more iterations.
...
- **Detailed report**: test plan/test cases changes, scripts created/updated (link to `script-changes.md`), network calls captured (link to `network-capture.md`), automated results (API/UI) vs. manual-only results, every behavior deviation from the Epic/Story with the specific acceptance criterion it violates, failure evidence (embed/link screenshot and video paths from `test-results/`), and an accessibility/security/performance summary (violation/alert counts by severity, k6 latency/error-rate/threshold results, linking `accessibility/report.md`, `security/report.md`, and `performance/report.md` from steps 5b/5c). On a re-run, also link `delta.md` and summarize how this run's results compare to the previous run.
