...
6. **Name the script after its module, never after a Jira key — and reuse/extend an existing one before creating a new file.** The target is `spec/performance/<module>.k6.js` (e.g. `pim.k6.js`, `leave.k6.js` — the module name from step 4's tagging rules, `@module:<module-name>`), not `<module>-<KEY>.k6.js`. First check whether `spec/performance/` already has a script for this module: if so, extend it — add this story's endpoint(s) as a new `check()` block commented with the Jira key that added it (see the existing file's comment style), don't create a duplicate or story-specific file. Only if no script exists yet for this module, copy `spec/performance/_template.k6.js` (per `quality.performance.template` in `config/qa-agent.config.json`) to `spec/performance/<module>.k6.js` and customize it. Either way, target this story's actual endpoint(s) — reuse the same endpoints already covered by this story's `Type: API` `@smoke` test cases (from `test-cases.md`/`network-capture.md`) rather than inventing new ones, and keep `options.vus`/`options.iterations` exactly as the template sets them (`1`/`5`). This script is a durable, shared repo asset under `spec/performance/`, not a per-run or per-story artifact — every story touching that module extends the same file.
7. **Resolve auth — fresh, not reused.** For endpoints requiring the admin session, re-run `spec/ui/auth.setup.ts` immediately before this step if there's any chance the session has since been invalidated (e.g. another Playwright run logged in again on this shared demo, which invalidates the previous session token), then read the cookie value from `playwright/.auth/admin.json` (the `orangehrm` cookie, or whatever cookie the app's storage state uses) and pass it to k6 as `AUTH_COOKIE`. A stale cookie causing every request to fail identically is the most common false "performance failure" — rule it out before treating a 100% failure rate as a real finding. If the target endpoints also need a CSRF token (check `network-capture.md` for a header like `X-CSRF-TOKEN`/`X-XSRF-TOKEN` on them), extract its value the same way and pass it as `AUTH_CSRF_TOKEN`. Don't hardcode credentials in the script itself.
...
9. **Run it** (single-user, 5 iterations — matches the template's `options`, but pass explicitly on the CLI too so it's unambiguous):
   ```
   BASE_URL=<env appUrl> AUTH_COOKIE=<cookie value> AUTH_CSRF_TOKEN=<csrf value, if needed> K6_SUMMARY_FILE=qa-artifacts/<KEY>/nonfunctional/run-<M>/performance/summary.json \
     k6 run --vus 1 --iterations 5 spec/performance/<module>.k6.js
   ```
   Docker fallback:
   ```
   docker run --rm -i -e BASE_URL -e AUTH_COOKIE -e AUTH_CSRF_TOKEN -e K6_SUMMARY_FILE -v "$PWD:/work" -w /work grafana/k6:latest \
     run --vus 1 --iterations 5 spec/performance/<module>.k6.js
   ```
